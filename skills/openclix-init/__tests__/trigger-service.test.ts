...
describe('TriggerService', () => {
  ...
  test('concurrent trigger() calls are serialized', async () => {
    ...
    const results = await Promise.all([
      service.trigger({ trigger: 'event_tracked', event, now: '2026-01-15T10:00:00.000Z' }),
      service.trigger({ trigger: 'event_tracked', event, now: '2026-01-15T10:00:01.000Z' }),
    ]);
    // Both should complete without error
    expect(results).toHaveLength(2);
    expect(results[0].evaluated_at).toBe('2026-01-15T10:00:00.000Z');
    expect(results[1].evaluated_at).toBe('2026-01-15T10:00:01.000Z');
  });
