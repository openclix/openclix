...
if ! has_command jq; then
  print_fail "jq is required but not installed. Install it from https://jqlang.github.io/jq/download/"
  exit 1
fi
...
if ! has_command npx; then
  print_fail "npx not available; mandatory ajv-cli schema validation cannot be run"
  exit 1
fi

ajv_log="$(mktemp)"
if npx --yes -p ajv-cli@5.0.0 -p ajv-formats@3.0.1 ajv validate --spec=draft2020 -c ajv-formats -s "${SCHEMA_PATH}" -d "${CONFIG_FILE}" >"${ajv_log}" 2>&1; then
...
if ! jq -e '.campaigns and (.campaigns | type == "object")' "${CONFIG_FILE}" >/dev/null 2>&1; then
  print_fail ".campaigns must exist and be an object"
fi

# 3d. All campaign keys are kebab-case
non_kebab_keys="$(jq -r '.campaigns | keys[] | select(test("^[a-z0-9]+(-[a-z0-9]+)*$") | not)' "${CONFIG_FILE}")"
if [[ -n "${non_kebab_keys}" ]]; then
  print_fail "Non-kebab-case campaign key(s) found: ${non_kebab_keys}"
fi

# 3e. Every campaign has type: "campaign"
bad_type_keys="$(jq -r '.campaigns | to_entries[] | select(.value.type != "campaign") | .key' "${CONFIG_FILE}")"
if [[ -n "${bad_type_keys}" ]]; then
  print_fail "Campaign(s) missing type: \"campaign\": ${bad_type_keys}"
fi

# 3f. Each trigger.type has its matching sub-object key
